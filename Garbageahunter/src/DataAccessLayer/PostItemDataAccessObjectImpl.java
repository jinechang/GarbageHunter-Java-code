	...
	public int addItem(PostItem item) {
		...
		try{
			...
			pstmt = con.prepareStatement( 
					"INSERT INTO PostItem (PostItemName,ItemName, Years_of_use,Post_by, "
					+ "Post_price, Post_date) VALUES(?, ?, ?, ?, ?,?)");
			...
			java.util.Date ud =item.getPostDate();
	...
	private List<PostItem> writetoobject(ResultSet rs){
		...
		try{
			...
			while (rs.next()){
				...
				postitem.setPostDate(rs.getDate("Post_date"));
				items.add(postitem);
			}
		}
